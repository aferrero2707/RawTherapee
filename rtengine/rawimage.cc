...
static void pseudoinverse_ (double (*in)[3], double (*out)[3], int size)
{
  double work[3][6], num;
  int i, j, k;
...
static void cam_xyz_coeff_ (float rgb_cam[3][4], float pre_mul[4], double cam_xyz[4][3], int colors)
{
  ...
  pseudoinverse_ (cam_rgb, inverse, colors);
  for (i=0; i < 3; i++)
    for (j=0; j < colors; j++)
...
int RawImage::loadRaw (bool loadData, unsigned int imageNum, bool closeFile, ProgressListener *plistener, double progressRange)
{
  ...
  try
  {
    ...
    char camera_makermodel[1000];
    g_strlcpy(camera_makermodel, make, sizeof(camera_makermodel));
    int maker_len = strlen(make);
    camera_makermodel[maker_len] = ' ';
    g_strlcpy(camera_makermodel+maker_len+1, model, sizeof(camera_makermodel)-maker_len-1);

    printf("RawImage::loadRaw(): make=\"%s\"  model=\"%s\"  makermodel=\"%s\"\n", make, model, camera_makermodel);
    ...
    // transfer black/white point values back to RT
    colors = 3; // TO BE FIXED!!! How to get number of colors from RawSpeed?
    black = 0; //img->raw_black_level;
    for(uint8_t i = 0; i < 4; i++)
    {
      cblack[i] = img->raw_black_level_separate[i];
      cam_mul[i] = img->wb_coeffs[i];
      pre_mul[i] = 0;
    }
    cam_mul[3] = cam_mul[1];
    maximum = img->raw_white_point;

    double cam_xyz[4][3]; cam_xyz[0][0] = NAN;
    printf("Getting default camera matrix for makermodel=\"%s\"\n",camera_makermodel);
    dt_dcraw_adobe_coeff(camera_makermodel, (double(*)[12])cam_xyz);

    cam_xyz_coeff_ (rgb_cam, pre_mul, cam_xyz, colors);
    ...
    left_margin = img->crop_x;
    top_margin = img->crop_y;
